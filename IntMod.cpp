...
#define MODINV_XCD			1	// ~97  kOps/s
#define MODINV_MONTGOMERY	2	// ~360 kOps/s
...
#define DSR62_DETMAN_1	3
#define DSR62_DETTMAN_2	4
#define DSR62_ALGO DSR62_PORIN
...
void Int::ModSub(const uint64_t a)
{
	Sub(a);
	if (IsNegative())
	{
...
#if MODINVP_ALGO == MODINV_MONTGOMERY
...
void Int::SetupField(const Int& n, Int* R, Int* R2, Int* R3, Int* R4)
{
	...
	// R = 2^(64*Msize) mod n
	Int Ri;
	Ri.MontgomeryMult(one, one); // Ri = R^-1
	_R.Set(Ri);                     // R  = R^-1
	_R2.MontgomeryMult(Ri, one);  // R2 = R^-2
	_R3.MontgomeryMult(Ri, Ri);    // R3 = R^-3
	_R4.MontgomeryMult(_R3, one); // R4 = R^-4
...
void Int::ModMulK1(const Int& a, const Int& b)
{
	...
	// 256*256 multiplier
	imm_umul((uint64_t*)a.bits64, b.bits64[0], r512);
	imm_umul((uint64_t*)a.bits64, b.bits64[1], t);
	...
	imm_umul((uint64_t*)a.bits64, b.bits64[2], t);
	...
	imm_umul((uint64_t*)a.bits64, b.bits64[3], t);
...
void Int::ModMulK1(const Int& a)
{
	...
	// 256*256 multiplier
	imm_umul((uint64_t*)a.bits64, bits64[0], r512);
	imm_umul((uint64_t*)a.bits64, bits64[1], t);
	c = _addcarry_u64(0, r512[1], t[0], r512 + 1);
	c = _addcarry_u64(c, r512[2], t[1], r512 + 2);
	c = _addcarry_u64(c, r512[3], t[2], r512 + 3);
	c = _addcarry_u64(c, r512[4], t[3], r512 + 4);
	c = _addcarry_u64(c, r512[5], t[4], r512 + 5);
	imm_umul((uint64_t*)a.bits64, bits64[2], t);
	c = _addcarry_u64(0, r512[2], t[0], r512 + 2);
	c = _addcarry_u64(c, r512[3], t[1], r512 + 3);
	c = _addcarry_u64(c, r512[4], t[2], r512 + 4);
	c = _addcarry_u64(c, r512[5], t[3], r512 + 5);
	c = _addcarry_u64(c, r512[6], t[4], r512 + 6);
	imm_umul((uint64_t*)a.bits64, bits64[3], t);
	c = _addcarry_u64(0, r512[3], t[0], r512 + 3);
	c = _addcarry_u64(c, r512[4], t[1], r512 + 4);
	c = _addcarry_u64(c, r512[5], t[2], r512 + 5);
	c = _addcarry_u64(c, r512[6], t[3], r512 + 6);
...
void Int::ModMulK1order(const Int& a)
{
	...
	imm_umul((uint64_t*)_O->bits64, ML, p.bits64);
	c = pr.AddC(p);
	memcpy(t.bits64, pr.bits64 + 1, 8 * (NB64BLOCK - 1));
	t.bits64[NB64BLOCK - 1] = c;
...
{
	// Compute modular inverse of this mop _O
	// 0 <= this < _O  , _O must be odd
	// Return 0 if no inverse
...
#if MODINVO_ALGO == MODINV_MONTGOMERY
