...
void printUsage() {
	...
	printf(" -sk startPrivKey: Start the search with a privKey\n");
...
void generateKeyPair(string seed, int searchMode, bool paranoiacSeed) {
	...
	printf("Priv : %s\n", Secp256K1::GetPrivAddress(compressed, privKey).c_str());
	printf("Pub  : %s\n", Secp256K1::GetPublicKeyHex(compressed, p).c_str());
...
#define CHECK_ADDR()												\
  ...
  p = Secp256K1::ComputePublicKey(fullPriv);						\
  cAddr = Secp256K1::GetAddress(addrType, compressed, p);			\
  if (cAddr == addr) {												\
    found = true;													\
    string pAddr = Secp256K1::GetPrivAddress(compressed, fullPriv);	\
    string pAddrHex = fullPriv.GetBase16();							\
    outputAdd(outputFile, addrType, addr, pAddr, pAddrHex);			\
  }

void reconstructAdd(string fileName, string outputFile, string privAddr) {
	...
	Int privKey = Secp256K1::DecodePrivateKey((char*)privAddr.c_str(), &compressed);
	...
	for (int i = 0; i < (int)lines.size(); i += 2) {
		...
		bool partialMode;
		Int partialPrivKey = Secp256K1::DecodePrivateKey((char*)partialPrivAddr.c_str(), &partialMode);
		if (privKey.IsNegative()) {
			printf("Invalid partialkey info file at line %d\n", i);
			exit(-1);
		}
...
int main(int argc, char* argv[]) {
	...
	while (a < argc) {
		...
		if (strcmp(argv[a], "-gpu") == 0) {
			...
			generateKeyPair(seed, searchMode, paranoiacSeed);
			exit(0);
		}
		else if (strcmp(argv[a], "-sp") == 0) {
			a++;
			string pub = string(argv[a]);
			startPuKey = Secp256K1::ParsePublicKeyHex(pub, startPubKeyCompressed);
			a++;
		}
			...
			string pub = string(argv[a]);
			bool isComp;
			Point p = Secp256K1::ParsePublicKeyHex(pub, isComp);
			printf("Addr (P2PKH): %s\n", Secp256K1::GetAddress(P2PKH, isComp, p).c_str());
			printf("Addr (P2SH): %s\n", Secp256K1::GetAddress(P2SH, isComp, p).c_str());
			printf("Addr (BECH32): %s\n", Secp256K1::GetAddress(BECH32, isComp, p).c_str());
			exit(0);
		}
			...
			if (priv[0] == '5' || priv[0] == 'K' || priv[0] == 'L') {
				k = Secp256K1::DecodePrivateKey((char*)priv.c_str(), &isComp);
			}
			else {
				k.SetBase16(argv[a]);
			}
			Point p = Secp256K1::ComputePublicKey(k);
			printf("PrivAddr: p2pkh:%s\n", Secp256K1::GetPrivAddress(isComp, k).c_str());
			printf("PubKey: %s\n", Secp256K1::GetPublicKeyHex(isComp, p).c_str());
			printf("Addr (P2PKH): %s\n", Secp256K1::GetAddress(P2PKH, isComp, p).c_str());
			printf("Addr (P2SH): %s\n", Secp256K1::GetAddress(P2SH, isComp, p).c_str());
			printf("Addr (BECH32): %s\n", Secp256K1::GetAddress(BECH32, isComp, p).c_str());
			exit(0);
			...
			a++;
			reconstructAdd(file, outputFile, priv);
			exit(0);
		}
	...
	VanitySearch* v = new VanitySearch(prefix, seed, searchMode, gpuEnable, stop, outputFile, sse,
		maxFound, rekey, caseSensitive, startPuKey, paranoiacSeed, startKey);
