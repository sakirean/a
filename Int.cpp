...
void Int::InitFactorials()
{
	// Factorials
	for (unsigned int i = 0; i < sizeof(Int::factorials) / sizeof(Int); ++i)
	{
		if (i == 0)
		{
			Int::factorials[i] = 1;
		...
		{
			Int::factorials[i] = Int::factorials[i - 1] * Int(i);
		}
	}
}
...
Int::Int(const std::string& str, unsigned int base)
{
	Clear();
	if (base == 16)
	{
...
Int::Int(const unsigned char* bytes)
{
	if (bytes)
	{
		Set32Bytes(bytes);
...
bool Int::operator==(const Int& r) const
{
	return IsEqual(r);
}
...
Int Int::operator+(const Int& r) const
{
	Int n(*this);
	n.Add(r);
	return n;
...
Int Int::operator>>(const uint32_t& r) const
{
	Int n(*this);
	n.ShiftR(r);
	return n;
...
Int& Int::operator>>=(const uint32_t& r)
{
	ShiftR(r);
	return *this;
}
...
bool Int::IsStrictPositive() const
{
	if (IsPositive())
	{
		return !IsZero();
...
void Int::BitwiseAnd(const Int& n)
{
	bits64[0] = bits64[0] & n.bits64[0];
	bits64[1] = bits64[1] & n.bits64[1];
	bits64[2] = bits64[2] & n.bits64[2];
	bits64[3] = bits64[3] & n.bits64[3];
	bits64[4] = bits64[4] & n.bits64[4];
}

void Int::BitwiseOr(const Int& n)
{
	bits64[0] = bits64[0] | n.bits64[0];
	bits64[1] = bits64[1] | n.bits64[1];
	bits64[2] = bits64[2] | n.bits64[2];
...
void Int::BitwiseXOR(const Int& n)
{
	bits64[0] = bits64[0] ^ n.bits64[0];
	bits64[1] = bits64[1] ^ n.bits64[1];
	bits64[2] = bits64[2] ^ n.bits64[2];
...
Int Int::Factorial(unsigned int i)
{
	if (i >= sizeof(factorials) / sizeof(Int))
	{
		printf("Factorial: Out of range!\n");
		exit(-1);
	}
	return factorials[i];
}
...
uint64_t Int::Mult(const uint64_t a)
{
	uint64_t carry;
...
{
	uint64_t carry;
	imm_mul((uint64_t*)a.bits64, b, bits64, &carry);
	return carry;
}
...
uint64_t Int::Mult(const Int& a, uint32_t b)
{
	uint64_t carry;
	imm_mul((uint64_t*)a.bits64, (uint64_t)b, bits64, &carry);
	return carry;
}
...
static uint64_t bitLength(uint64_t dw)
{
	uint64_t mask = 0x8000000000000000;
	uint64_t b = 0;
	while (b < 64 && (mask & dw) == 0) {
		b++;
		mask >>= 1;
	}
	return b;
...
int Int::GetSize8() const
{
	int i = NB08BLOCK - 1;
	while (i > 0 && bits08[i] == 0)
	{
		i--;
	}
	return i + 1;
...
int Int::GetSize16() const
{
	int i = NB16BLOCK - 1;
	while (i > 0 && bits16[i] == 0)
	{
		i--;
	}
	return i + 1;
...
bool Int::IsProbablePrime()
{
	...
	for (uint64_t i = 0; i < k; i++)
	{
		...
		Int x;
		x.SetInt32(0);
		while (x.IsLowerOrEqual(one) || x.IsGreaterOrEqual(N1))
		{
			x.Rand(nbBit);
		}
		x.ModExp(Q);
